...
export class GpuProfiler {
    ...
    /** All timed passes, largest average first. Allocates — call at UI refresh rate, not per frame. */
    public get passes(): PassTiming[] {
        return [...this._timings.values()].sort((a, b) => b.avgMs - a.avgMs);
    }
    ...
    private _record(name: string, ms: number): void {
        ...
        if (!t) {
            t = { name, ms, avgMs: ms, maxMs: ms, samples: 0 };
