...
import { PropertyTable, PropertyRow, Slider, Hint } from '../../../components/ui'
...
export default function SkyLightEditor(props: { node: SkyLightNode }) {
  ...
  return (
    <Collapsable title='Sky Light' icon={<SkyIcon />} persistKey='skyLight'>
      <div className='w-full p-2'>
        <PropertyTable columns={['45%', '55%']}>
          ...
          <PropertyRow label='Cloud Response' divider={false}>
            <Slider min={0} max={1} step={0.05} value={cloudResponse}
                    onChange={(v) => apply({ cloudResponse: v })} />
          </PropertyRow>
        </PropertyTable>
        <Hint className='mt-2'>
          Cloud Response is how much the scene's clouds change the lighting. At 1 an overcast sky takes
          the sun's strength and its warm cast and hands both to the sky, which also goes flat — so the
          scene reads white and shadowless. At 0 the clouds are drawn but light nothing.
        </Hint>
        <Hint className='mt-2'>
          Lights the scene with the sky itself — a surface facing up receives the zenith, one facing a
          sunset receives the sunset. Works from the Sky Atmosphere or from a Skybox, whichever the
          scene has. Add one and there is no need for a light probe just to get ambient light.
        </Hint>
