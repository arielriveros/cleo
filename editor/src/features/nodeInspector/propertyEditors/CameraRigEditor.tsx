...
import { Button, Select, Slider, Toggle, cn, hintClass, labelClass, sectionTitleClass } from '../../../components/ui'
...
export default function CameraRigEditor(props: { node: CameraRigNode }) {
  ...
  const axis = (label: string, k: keyof RigState, step = 0.05) => (
    <div className='mb-2'>
  ...
  const dropdown = (label: string, k: keyof RigState, options: { value: string, label: string }[]) => (
  ...
  return (
    <Collapsable title='Camera Rig' icon={<RigIcon />} persistKey='cameraRig'>
      <div className='w-full p-2'>
        <div className={cn(hintClass, 'mb-2')}>
          The rig drives its child camera's position and rotation every frame — edit the boom here
          rather than moving the camera directly.
        </div>

        {header('Follow (drives position)')}
        <div className='mb-2'>
          <label className={labelClass}>Follow Target</label>
          <NodeRefInput value={state.followId} scene={editorScene} filter={notSelfOrDescendant}
            onChange={(id) => apply({ followId: id })} />
          {!state.followId &&
            <div className={cn(hintClass, 'mt-1')}>No target: the rig's own position is the pivot.</div>
          }
        </div>
        ...
        {axis('Damping (seconds)', 'followDamping', 0.01)}
        {dropdown('Damping Axes', 'followDampingSpace', [
          { value: 'world', label: 'World' },
          { value: 'rig', label: "Rig (behind / side / up)" },
        ])}
        <div className={hintClass}>0 seconds is rigid; higher values lag further behind.</div>
        ...
        {state.aimMode === 'orbit' && <>
          ...
          <div className={cn(hintClass, 'mb-1')}>
            Feed input from a script: <code>rig.addYaw(dx)</code> / <code>rig.addPitch(dy)</code>.
          </div>
        </>}
        ...
        {header('Spring Arm')}
        {slider('Arm Length', 'armLength', 0, 30, 0.1)}
        {axis('Socket Offset', 'socketOffset')}
        <div className={hintClass}>Arm length 0 gives a first-person camera at the pivot.</div>
        ...
        <div className={cn(hintClass, 'mt-1')}>
          Shake is visible in Play mode. From a script: <code>rig.shake(0.6)</code>.
        </div>
      </div>
    </Collapsable>
  )
