...
async function runBackend(backend) {
  ...
  await js(PROBE);
  // `CLEO_NO_MARCH=1` switches the terrain's parallax march off for the run, so its cost can be read as
  // a difference rather than guessed at. Zeroing the depth is the one switch that removes the march and
  // nothing else — the weights, the blend and every texture stay as they are. See `__setMarchDepth`.
  if (process.env.CLEO_NO_MARCH === '1')
    console.log('   march disabled: u_dispScale0 = '
